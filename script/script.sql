...
insert into bbs(title,contents,uid)
...
insert into bbs(title, contents, uid)
select title, contents, uid from bbs;
...
date_format(r.updateDate, '%Y년 %m월%d일 %T')as fmtUpdateDate
...
update bbs set replyCnt = (select count(*) from reply where bbs.bid=reply.bid)
where bid > 0;
